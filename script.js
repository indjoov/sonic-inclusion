// script.js (FULL REPLACE) — Smooth hypnotisch + space drift + sharper cage + Record fix
...
let world = null;      // cage/sigil/rings in here
let starPoints = null; // stars in scene space

// Cage
let cage = null;
let cageGeoBase = null;     // IcosahedronGeometry (positions we deform)
let cagePosAttr = null;     // BufferAttribute reference
let cagePosBase = null;     // Float32Array copy of original positions
let cagePhase = 0;
let cageRebuildCounter = 0;
...
/* ================= MODERN HUD (FIXED + CENTERED) ================= */

function fixedCenteredWidth(maxPx = 980, pad = 16) {
  return `
    position: fixed;
    left: 50%;
    transform: translateX(-50%);
    width: min(${maxPx}px, calc(100vw - ${pad * 2}px));
...
recBtn.style.cssText = `
  ...
  opacity: .98;
`;
recBtn.addEventListener("pointerdown", (e) => e.stopPropagation(), { passive: true });
...
`;
engineToggle.addEventListener("pointerdown", (e) => e.stopPropagation(), { passive: true });

hud.appendChild(recBtn);
hud.appendChild(engineToggle);
document.body.appendChild(hud);

/* ================= ENGINE PANEL (FIXED + CENTERED) ================= */
...
function initThree() {
  ...
  // World group moves through space
  world = new THREE.Group();
  scene.add(world);

  // Stars in scene space: "universe"
  starPoints = makeStars(2100, 140);
  ...
  // Composer with MSAA render target on WebGL2 for sharper lines
...
function makeCage() {
  // Higher detail base geometry (we will deform it smoothly)
  cageGeoBase = new THREE.IcosahedronGeometry(5.2, 4);
  cagePosAttr = cageGeoBase.getAttribute("position");
  cagePosBase = new Float32Array(cagePosAttr.array.length);
  cagePosBase.set(cagePosAttr.array);

  // Start with wireframe built from base
  const wire = new THREE.WireframeGeometry(cageGeoBase);
...
function initRings() {
  ...
  for (let i = 0; i < 8; i++) {
    const g = new THREE.RingGeometry(2.6, 2.9, 140);
...
function triggerRingPulse(intensity = 1) {
  ...
  r.mesh.material.opacity = 0.80 * P.ringStrength;
}
...
function spawnGhostBurst(count = 3, intensity = 1, snapFlash = 1) {
  ...
  for (let k = 0; k < useCount; k++) {
    ...
    g.ink.material.opacity = 0.20 + 0.18 * intensity;
    g.glow.material.opacity = 0.38 + 0.52 * snapFlash;
...
function loadSigilLayers(url) {
  ...
  fetch(url)
    ...
    .then(svgText => {
      ...
      img.onload = () => {
        // higher res => cleaner ink on mobile
        const size = 1536;
        ...
        // white background for alpha-key
...
// smooth pseudo-noise without extra libs
function softNoise(x, y, z) {
  return (
    Math.sin(x * 1.23 + z * 0.97) * 0.5 +
    Math.sin(y * 1.11 + x * 0.83) * 0.3 +
    Math.sin(z * 1.37 + y * 0.73) * 0.2
  );
}

function deformCage(dt, bass, mid, snare) {
  if (!cageGeoBase || !cagePosAttr || !cagePosBase) return;

  // hypnotic morph: mostly bass + mid, snare only micro
  const sensMul = sens ? parseFloat(sens.value) : 1;
  const amount = P.morphAmt * (0.55 + 0.45 * Math.min(1, bass * 1.2)) * sensMul;
  const micro = (reducedMotion ? 0 : 0.10) * Math.min(1, snare * 1.4);

  cagePhase += dt * (0.35 + P.morphSpeed * 0.55);

  const arr = cagePosAttr.array;
  const base = cagePosBase;
  ...
  for (let i = 0; i < arr.length; i += 3) {
    ...
    const n =
      softNoise(
        nx * 2.2 + cagePhase,
        ny * 2.2 + cagePhase * 0.9,
        nz * 2.2 - cagePhase * 0.7
      );

    // hypnotic breathing: slow bass, mid adds subtle ripples
    const breathe = 1 + 0.05 * Math.sin(cagePhase * 0.8) + bass * 0.10;
    const ripple = (0.35 + mid * 0.45) * n;

    const disp = (amount * ripple) + micro * (Math.random() - 0.5) * 0.04;

    arr[i]     = bx * breathe + nx * disp;
    arr[i + 1] = by * breathe + ny * disp;
    arr[i + 2] = bz * breathe + nz * disp;
  }

  cagePosAttr.needsUpdate = true;

  // Rebuild wireframe occasionally (keeps performance ok)
  cageRebuildCounter++;
  const rebuildEvery = 6; // every ~6 frames
  if (cageRebuildCounter % rebuildEvery === 0 && cage?.geometry) {
    const old = cage.geometry;
    cage.geometry = new THREE.WireframeGeometry(cageGeoBase);
    old.dispose();
  }
}
...
function loop() {
  ...
  // Stars: NOT music-driven. Create slow space drift + subtle twinkle
  if (starPoints) {
    const base = P.starsOpacity;
    const tw = base + 0.03 * Math.sin(tNow * 0.00065);
    const slider = partEl ? parseFloat(partEl.value) : 10; // 0..30
    const add = Math.max(0, Math.min(0.20, 0.0065 * slider));
    starPoints.material.opacity = Math.max(0, Math.min(0.70, tw + add));
  ...
  // World: slow drifting “in space” (independent of music)
  if (world && !reducedMotion) {
    const t = tNow * 0.00022;
    world.position.x = Math.sin(t * 1.15) * 0.55;
    world.position.y = Math.cos(t * 0.92) * 0.35;
    world.position.z = Math.sin(t * 0.70) * 0.75;

    world.rotation.y = t * 0.45;
    world.rotation.x = Math.sin(t * 0.78) * 0.10;
  }

  // Cage: color/opacity + morph (smooth hypnotic)
  if (cage) {
    ...
    if (mode === "grayscale") {
      cage.material.color.setHex(0xe8e8e8);
    } else if (mode === "energy") {
      cage.material.color.setHSL(hue, 0.85, 0.32 + energy * 0.30);
    } else {
      cage.material.color.setHSL(hue, 0.78, 0.52);
    }

    // slightly lower bloom reliance -> sharper cage
    cage.material.opacity = P.cageOpacityBase + bassSm * 0.12 + snapFlash * 0.05;

    // micro rotation only (avoid “cheap wobble”)
    const drift = reducedMotion ? 0 : 0.00075;
    cage.rotation.y += drift;
    cage.rotation.x += drift * 0.55;

    // Morph geometry (smooth)
    deformCage(dt, bassSm, midSm, snareSm);
  }

  // Object zoom (no camera zoom => stars feel like space)
  const zoomInt = zoomEl ? (parseFloat(zoomEl.value) / 100) : 0.18;
  const objZoom = 1 + bassSm * (0.30 * zoomInt) + snapFlash * 0.03;

  // Sigil
  if (sigilGroup && sigilBase && sigilGlow) {
    const mode = palette?.value || "hue";
    sigilBase.material.opacity = P.sigilInk + bassSm * 0.05;
    ...
    if (mode === "grayscale") {
      ...
      const cyan = new THREE.Color(0x00d4ff);
      const purple = new THREE.Color(0x7c4dff);
      glowColor = cyan.clone().lerp(purple, Math.min(1, snapFlash * 1.05));
    }
    sigilGlow.material.color.copy(glowColor);

    const aura = P.glowBase + bassSm * P.glowBass;
    const flash = snapFlash * P.glowSnap;
    sigilGlow.material.opacity = Math.max(0.18, Math.min(0.92, aura + flash));

    // tiny hypnotic sway (no random jitter)
    if (!reducedMotion) {
      const tt = tNow * 0.001;
      sigilGroup.rotation.y = 0.22 + Math.sin(tt * 0.95) * 0.018;
      sigilGroup.rotation.x = -0.18 + Math.sin(tt * 0.80) * 0.012;
    }
  ...
  for (const r of ringPool) {
    ...
    const flick = 0.92 + 0.08 * Math.sin(tNow * 0.02);
    r.mesh.material.opacity = (1 - p) * 0.80 * flick * P.ringStrength;
  ...
  // Bloom reacts, but restrained (sharper cage)
  if (bloomPass) {
    bloomPass.strength = P.bloomStrength + bassSm * 0.22 + snapFlash * 0.35;
  }
...
async function startRecording() {
  // Important: recording must start AFTER user gesture. This click is a gesture.
  await initEngine();

  if (!canvas.captureStream) {
    setStatus("❌ Recording not supported on this browser (no canvas.captureStream)");
    return;
  }

  // resume audio in case it is suspended
  try { await engine.resume(); } catch {}

  const fps = 60;
  const canvasStream = canvas.captureStream(fps);

  // add audio track from master tap (if available)
  const out = audioRecordDest?.stream;
  if (out && out.getAudioTracks().length) {
    try { canvasStream.addTrack(out.getAudioTracks()[0]); } catch {}
  }
  ...
  try {
    ...
    console.error(err);
    setStatus("❌ MediaRecorder init failed (browser limitation)");
    return;
  }
...
recBtn.addEventListener("click", async () => {
  if (overlay.style.display !== "none" && !engineReady) {
    setStatus("👆 Click the center overlay once to unlock audio, then record.");
    return;
  }

...
function savePreset(slot) {
    const data = {
        sens: sens?.value, hue: hueEl?.value, zoom: zoomEl?.value,
...
function initNebulaBackground() {
    ...
    nebulaMaterial = new THREE.ShaderMaterial({
        uniforms: {
            time: { value: 0 }, bass: { value: 0 },
            color1: { value: new THREE.Color(0x0a001a) }, 
            color2: { value: new THREE.Color(0x002233) }  
        },
        vertexShader: `
            varying vec2 vUv;
...
async function initEngine() {
  try {
    ...
    try { 
        ...
        console.warn("Recording dest not supported on this browser.");
    }
    ...
    setStatus("❌ Error starting engine. Check console.");
...
fileBtn?.addEventListener("click", async () => { if(currentMode==="idle") await initEngine(); fileInput?.click(); });
